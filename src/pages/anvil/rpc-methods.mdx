...
| `eth_getBlockAccessList`, `eth_getBlockAccessListByBlockHash`, `eth_getBlockAccessListByBlockNumber`, `eth_getBlockAccessListRaw` | Return the EIP-7928 block access list for a block as JSON, or RLP for the raw variant. Anvil records the list for every Amsterdam block it mines; pre-Amsterdam blocks return `null`, and when forking, blocks below the fork block are answered by the upstream node. |
