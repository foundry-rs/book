...
| `snapshots` | path | `snapshots` | Directory used for gas and CPU snapshot files. |
...
| `cpu_snapshot_check` | boolean | `false` | Checks generated CPU snapshots against stored snapshots. |
| `cpu_snapshot_emit` | boolean | `true` | Writes CPU snapshots to disk. |
...
#### CPU snapshot controls

CPU snapshots record elapsed thread CPU time in nanoseconds and are stored as
`<snapshots>/cpu/<group>.json`. When `cpu_snapshot_check` is enabled, `forge test`
fails if a recorded value differs from an existing value on disk. When
`cpu_snapshot_emit` is enabled, the values from the run are written after the
check. The settings can also be controlled by `FORGE_CPU_SNAPSHOT_CHECK` and
`FORGE_CPU_SNAPSHOT_EMIT`, or overridden with the corresponding
`--cpu-snapshot-check=<boolean>` and `--cpu-snapshot-emit=<boolean>` flags.

```toml
[profile.default]
cpu_snapshot_check = true
cpu_snapshot_emit = false
```
