...
derived from your eligible target ABIs. Each function has random-argument and dictionary-backed
argument productions; Foundry still generates concrete arguments, senders, and transaction values
locally. ABI-derived choices do not infer semantic relationships such as an amount equaling a balance.
...
:::warning
Enabling `jev` opts in to sending eligible function signatures, types, local target ordinals, and
recent execution outcomes to OpenRouter. Foundry does not send source, addresses, calldata,
storage, or dictionary contents. Requests incur provider charges.
:::
...
This experimental mode supports at most 64 eligible functions and 32 requests per worker per
invariant test group, with two-second HTTP deadlines. Workers wait for each batch; include that
