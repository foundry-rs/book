...
- [`block-number-across-roll`](/forge/linting/block-number-across-roll) — Flags `block.number` values read before and used after `vm.roll`; use `vm.getBlockNumber()` to capture the intended block number.
- [`block-timestamp-across-warp`](/forge/linting/block-timestamp-across-warp) — Flags `block.timestamp` values read before and used after `vm.warp`; use `vm.getBlockTimestamp()` to capture the intended timestamp.
...
- [`tx-origin`](/forge/linting/tx-origin) — Flags use of `tx.origin` in authorization checks, which a malicious contract can bypass when called by a legitimate owner.
...
- [`block-timestamp`](/forge/linting/block-timestamp) — Flags comparisons using `block.timestamp` for review against the target chain's timestamp and transaction-inclusion guarantees.
...
- [`empty-block`](/forge/linting/empty-block) — Flags empty function bodies, excluding constructors, `receive`/`fallback`, virtual functions, and payable functions without return parameters.
...
- [`missing-events-arithmetic`](/forge/linting/missing-events-arithmetic) — Flags protected functions that update integer state used in arithmetic by an unprotected function without emitting an event.
- [`missing-zero-check`](/forge/linting/missing-zero-check) — Flags functions and constructors that use an `address` parameter in a state write or value transfer without checking for the zero address.
- [`msg-value-loop`](/forge/linting/msg-value-loop) — Flags `msg.value` reads inside loops executed by externally callable payable functions.
- [`reentrancy-events`](/forge/linting/reentrancy-events) — Flags events emitted after an external call, where reentrancy can change the log order that off-chain consumers rely on.
- [`require-revert-in-loop`](/forge/linting/require-revert-in-loop) — Flags `require` calls and `revert` statements inside loops, where one failure can revert the whole batch.
...
- [`costly-loop`](/forge/linting/costly-loop) — Flags storage writes inside loops; accumulating a result locally can reduce repeated storage work when it preserves behavior.
...
- [`external-function`](/forge/linting/external-function) — Flags `public` functions without internal callers as candidates for `external` visibility and `calldata` parameters; verify callers and inheritance before changing them.
...
- [`write-after-write`](/forge/linting/write-after-write) — Flags storage writes overwritten before being read; removing one can save gas if the compiler has not already eliminated it and its value expression has no required side effects.
