...
`assert()` is meant for invariant checking, conditions that should _never_ be false if the contract is correct. `require()` is for input validation and conditions that may legitimately fail at runtime. Mixing a state mutation into an `assert()` argument conflates these two concerns:
