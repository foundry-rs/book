...
and `return`/`stop`/`selfdestruct`, which halt _successfully_ without running the function body and
are therefore flagged). A regular function call that _might_ revert (e.g. `require(...)`, an
