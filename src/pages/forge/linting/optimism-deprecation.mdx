---
description: Usage of deprecated Optimism predeploy or GasPriceOracle function
---
...
Calling removed predeploys will revert every time — there is no code at those addresses.
...
function legacyFee() external view returns (uint256) {
...
accessed through a local variable alias:

```solidity
// Also flagged — alias is traced back to the literal
IGasPriceOracle gpo = IGasPriceOracle(0x420000000000000000000000000000000000000F);
gpo.overhead();
```

Local variable aliases initialised directly from a literal are tracked; state variables and reassigned locals are not.
