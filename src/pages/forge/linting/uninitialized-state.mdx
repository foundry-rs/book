...
- _Storage aliases_: `Foo storage f = bar; f.x = 1;` is not detected as a write to `bar`.
- _Storage-parameter calls (partial)_: the lint detects when a state variable is passed as
...
- _Member calls_: any member call whose receiver is a state variable (e.g.
