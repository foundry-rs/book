...
For sender selection, CREATE2, library linking, nonces, simulation, and resume semantics, see [How scripting works](/forge/scripting-internals).
...
Or derive the sender from a private key read from the environment:
...
The address overload selects a transaction sender but does not supply a signing key. Provide a matching wallet when broadcasting, or use `--unlocked` with a node that controls that account. The private-key overload derives the address and adds the key to Forge's script wallets.
