...
Add `--legacy` to request a legacy transaction instead of an EIP-1559 transaction through your browser wallet:
...
On Ethereum, Foundry passes the explicit legacy transaction type (`0x0`) and `gasPrice` to the wallet. You can set the gas price with `--gas-price`; otherwise, Foundry estimates it through the CLI RPC. Review the fees in the wallet before approving. See the [`cast send` reference](/reference/cast/send) for transaction options.
