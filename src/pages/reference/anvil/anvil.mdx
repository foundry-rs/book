---
description: "A fast local Ethereum development node Examples: - anvil (start a local node on 127.0.0.1:8545) - anvil --fork-url $RPC_URL (fork the latest state of a live network) - anvil --block-time 12 (mine a new block every 12 seconds) - anvil --state state.json (load state if it exists and dump it on exit)"
---
