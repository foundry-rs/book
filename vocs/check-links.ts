...
async function getAllMarkdownFiles(): Promise<string[]> {
  const files = await Array.fromAsync(glob(CONFIG.FILE_PATTERNS, { cwd: CONFIG.DOCS_DIR }));
  return files.map((file) => join(CONFIG.DOCS_DIR, file));
}

async function getAllPublicAssets(): Promise<string[]> {
  return Array.fromAsync(glob("**/*", { cwd: CONFIG.PUBLIC_DIR }));
}

function buildFilePathMap(files: Array<string>, publicAssets: Array<string>): Set<string> {
...
function extractLinksFromMarkdown(content: string): Array<{ link: string; line: number }> {
...
function extractHtmlLinks(line: string, lineNumber: number): Array<{ link: string; line: number }> {
...
function validateLink(link: string, sourceFile: string, pathMap: Set<string>): string | null {
...
function validateAbsolutePath(linkPath: string, pathMap: Set<string>): string | null {
  const variations = [
    linkPath,
    linkPath.slice(1), // Remove leading slash
    linkPath.replace(/\/$/, ""), // Remove trailing slash
    linkPath.slice(1).replace(/\/$/, ""), // Remove both
  ];
...
function displayResults(report: LinkCheckReport) {
  ...
  if (report.brokenLinks.length > 0) LinkCheckReporter.printBrokenLinks(report.brokenLinks);
  else console.log("\n✅ All links are valid!");
}
