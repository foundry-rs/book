...
- **(Advanced)** If your Anvil version supports it, start with
  `--disable-pool-balance-checks` to bypass transaction-pool balance checks
  so the second tx can be accepted while the funding tx is still pending:

  ```sh
  anvil --disable-pool-balance-checks
  ```

  _Note:_ This is intended for local testing only; it relaxes safety checks and may admit transactions that would be rejected on mainnet. See `anvil --help` for details.

You can temporarily enable automine via `anvil_setAutomine` or manually mine a block with `anvil_mine`.
