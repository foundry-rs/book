...
- Type: boolean
- Default: true
- Environment: `FOUNDRY_INVARIANT_SHOW_OPTIMIZER`

Flag indicating whether to display optimizer metrics (best value found, sequence) for invariant functions that return `int256`. When an invariant function returns `int256`, Foundry automatically enables optimization mode to find the sequence that maximizes the return value.
