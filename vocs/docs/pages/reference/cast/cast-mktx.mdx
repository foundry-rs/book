...
import WalletOptionsRaw from "../common/wallet-options-raw.mdx";
import WalletOptionsKeystore from "../common/wallet-options-keystore.mdx";
import WalletOptionsHardware from "../common/wallet-options-hardware.mdx";
...
3. Sign a transaction that calls a function that expects a `struct`:
   ...
   Structs are encoded as tuples (see [struct encoding](#TODO))
...
[cast](/reference/cast/cast.mdx), [cast publish](/reference/cast/cast-publish.mdx), [cast send](/reference/cast/cast-send.mdx), [struct encoding](#TODO)
