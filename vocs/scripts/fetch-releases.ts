    ...
    while (hasMore && page <= 5) {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          console.warn(
            "⚠️  GitHub API rate limit may have been reached. Consider setting GITHUB_TOKEN environment variable.",
          );
        }
        return [];
...
async function fetchReleaseByTag(tag: string): Promise<GitHubRelease | null> {
  try {
    const response = await fetch(
      `https://api.github.com/repos/foundry-rs/foundry/releases/tags/${tag}`,
      {
        headers: {
          Accept: "application/vnd.github+json",
          "X-GitHub-Api-Version": "2022-11-28",
...
function createMainReleasePage(
  latestRelease: ProcessedRelease,
  previousReleases: ProcessedRelease[],
): string {
...
async function main() {
  try {
    ...
    const latestRelease = publishedReleases.find(
      (r) => !r.tag_name.toLowerCase().includes("nightly") && r.tag_name !== "stable",
    );
    ...
    const nightlyReleases = publishedReleases.filter((r) => r.tag_name.startsWith("nightly-"));
