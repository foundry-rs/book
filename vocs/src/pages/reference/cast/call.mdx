...
Options:
      ...
      --hardfork <HARDFORK>
          The EVM hardfork to use. Can only be used with `--trace`
...
Transaction options:
      ...
      --nonce-key <NONCE_KEY>
          ...
          Allows multiple transactions with the same nonce but different keys to
...
Wallet options - access key:
      --access-key <PRIVATE_KEY>
          Use an access key to sign on behalf of a root account.
          
          The access key is a delegated key that can sign transactions for the
          root account. Requires --root-account to specify the account the key
          signs for.
      ...
      --root-account <ADDRESS>
          ...
          Required when using --access-key. This is the account that holds the
          funds and whose nonce is used for the transaction.
